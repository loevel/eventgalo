...
export async function notifyWaitlist(env: Env, categoryId: string, freedSlots: number): Promise<void> {
  ...
  const entries = await env.DB.prepare(
    `SELECT w.id, w.name, w.email, c.name AS category_name, e.title AS event_title, e.public_slug
     FROM waitlist w
    ...
    .all<{ id: string; name: string; email: string; category_name: string; event_title: string; public_slug: string }>();
